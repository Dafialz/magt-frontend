  ...
  (function attachSoftGuard(u){
    ...
    try {
      u.onModalStateChange?.((s) => {
        ...
        if (isOpen) { window.__tc_open = true; try{document.documentElement.classList.add('tc-open');}catch{} 
          // 3.5 секунди "м'якого" лок-вікна після відкриття (трохи довше для мобільних)
          lockUntil = Date.now() + 3500;
          log("modal opened (soft-lock)");
        } else {
          window.__tc_open = false; try{document.documentElement.classList.remove('tc-open');}catch{}; log("modal closed");
        }
      });
    } catch {}
